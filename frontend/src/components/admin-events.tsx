...
export function AdminEvents({ isAdmin }: { isAdmin: boolean }) {
  ...
  return <div className="events-workspace">
    ...
    <div className="event-toolbar"><input aria-label="Filter events by page or control" placeholder="Find a page or clicked control" value={search} onChange={e=>setSearch(e.target.value)} /><select aria-label="Date range" value={days} onChange={e=>setDays(Number(e.target.value))}><option value={1}>Today / last 24h</option><option value={7}>Last 7 days</option><option value={30}>Last 30 days</option></select><select aria-label="Device filter" value={device} onChange={e=>setDevice(e.target.value)}><option value="all">All devices</option><option value="mobile">Mobile</option><option value="desktop">Desktop</option></select><button className="button small-button" disabled={!isAdmin || loading} onClick={()=>setRefresh(r=>r+1)}><RefreshCw size={15}/>Refresh</button></div>
